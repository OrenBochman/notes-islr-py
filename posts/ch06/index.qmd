...
The chapter is 60 pages long and covers the following topics:
...
There is lots of material in this chapter. Looking over it all I have a couple of pointers that seem to be outside of the scope of the book.

Model Selection makes more sense to me when we are in a Bayesian setting c.f. [@probmods2] where the authors explain
about the [bayesian occam's razor](http://probmods.org/chapters/occams-razor.html). The more complex the model, the more data we need to fit it. In a Bayesian setting, we can use the posterior predictive distribution to evaluate how well a model fits the data. This is not covered in this chapter.

The authors treat Shrinkage is a way to regularize the model. This is a way to reduce the variance of the model. However there are some additional facets or benefits to shrinkage. In hierarchical models, shrinkage is a way to borrow strength from the data and we can also use prior distributions to shrink the model. Priors and the ability to transfer probability mass from known to unknown parameters in a hierarchical model can become a form of *transfer learning*. As we develop insights into the data generating process, we may wish to add in out subjective beliefs about its behavior. USing an inductive bias, as embodied in hierarchical models, can help us to build models that can generalize faster or give useful predictions with fewer data points. This is called few-shot learning. Finally real world data is often sparse and unbalanced. Shrinkage can help us make more educated guesses what is going on in the regions of the data we have less observation.

::: {.callout-tip}  

## TL;DR - Model Selection {.unnumbered}

![Model Selection in a nutshell](/images/in_a_nutshell.jpg)
...
![Best Subset Selection](alg6.1.png){.column-margin}
...
![Forward Stepwise Selection](alg6.2.png){.column-margin}
...
![Backward Stepwise Selection](alg6.3.png){.column-margin}
