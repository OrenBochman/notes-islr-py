...
1. **Simple Linear Regression**: Modeling the relationship between a single input (*predictor*) variable and an output (*response*) variable using a straight line.
...
1. **Qualitative Predictors**: When we want to play with non0numerical predictors, e.g. representing  color categoris, we can encode them as numerical values or better yet we can use **dummy variables** that track if a category is on :arrow_forward: or off :stop_button:.
Incorporating categorical variables into regression models using *dummy variables*. 
...
1. **Model Diagnostics** :thermometer: : Identifying and addressing potential issues :face_with_thermometer: like non-linearity, heteroscedasticity, outliers, high leverage points, and collinearity.
1. **heteroscedasticity** :space_invader: - when the response spreads out more as the predictors increase. You can usually eyeball :roll_eyes: this by looking at the residuals vs. fitted values plot.
1. **outliers** :alien: - observations that don't fit the general pattern of the data. If they are mistakes we can toss them out :wastebasket:. But is they are for real, we call them **high leverage points** :ghost:.
1. **collinearity** :zombie: - when *predictors* are highly correlated, leading to unstable coefficient estimates. The **VIF** (Variance Inflation Factor) helps detect collinearity.
